...
int main()
{
    ...
    while (!q.empty())
    {
        auto c = q.top();
        q.pop();
        if (get<1>(c) == m && get<2>(c) == m)
            break;
        if (dist[get<1>(c)][get<2>(c)] < get<0>(c))
            continue;
        if (cc == -1)
        {
            for (int i = 0; i < 4; i++)
            {
                ...
                if (get<1>(c) + cx < 1 || get<1>(c) + cx > m || get<2>(c) + cy < 1 ||
                    get<2>(c) + cy > m)
                    continue;
                if (grid[get<1>(c) + cx][get<2>(c) + cy] == -1)
                    continue;
                int cg = 0;
                if (grid[get<1>(c) + cx][get<2>(c) + cy] != get<3>(c))
                    cg = 1;
        ...
        else
        {
            for (int i = 0; i < 4; i++)
            {
                ...
                {
                    int cg = 0;
                    if (grid[get<1>(c) + cx][get<2>(c) + cy] != cc)
                        cg = 1;
                    if (dist[get<1>(c) + cx][get<2>(c) + cy] == -1 ||
                        dist[get<1>(c) + cx][get<2>(c) + cy] > get<0>(c) + cg)
                    {
